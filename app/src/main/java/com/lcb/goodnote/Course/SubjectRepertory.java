...
public class SubjectRepertory {
    ...
    public static List<MySubject> loadByLitePal(){
        return parsebyLitePal();//直接加入所有数据库中的内容
    }
    public static List<MySubject> parsebyLitePal() {
        List<MySubject> courses = new ArrayList<>();
        List<CourseData> clist = LitePal.findAll(CourseData.class);
        try {
            for(int i=0;i<clist.size();i++){
                String term=clist.get(i).getTerm();// 0 学期 "2017-2018学年秋"
                String name=clist.get(i).getName();// 3 课程名 "计算机组成原理"
                String room= clist.get(i).getRoom();// 地点
                String teacher=clist.get(i).getTeacher();// 8 教师
                List<Integer> weeks = clist.get(i).getWeekList();
                int day = clist.get(i).getDay();
                int start = clist.get(i).getStart();
                int step =  clist.get(i).getStep();
                courses.add(new MySubject(term,name, room, teacher, weeks, start, step, day, -1,null));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                String term=array2.getString(0);// 0 学期 "2017-2018学年秋"
//                String name=array2.getString(3);// 3 课程名 "计算机组成原理"
//                String teacher=array2.getString(8);// 8 教师
//                if(array2.length()<=10){
...
//                }
//                String room=array2.getString(16)+string;// 地点
//                String weeks=array2.getString(11); // 周数
//                int day,start,step;
//                try {
//                    day=Integer.parseInt(array2.getString(12));//周几上
//                    start=Integer.parseInt(array2.getString(13));//开始上课的节次
//                    step=Integer.parseInt(array2.getString(14));//上几节
//                } catch (Exception e) {
//                    day=-1;
//                    start=-1;
//                    step=-1;
